...
def draw_wall(Ha, Hw, Hp, Th, Tt, Lh, Lt, Tsb, beta):
    ...
    # =======================
    # ACTIVE SOIL (CORRECT SLOPED TOP)
    x_left = x0 + gap
    x_right = x0 + Lh - gap

    y_bottom = y0 + base_h + gap
    y_top_left = y_bottom + Ha
    y_top_right = y_top_left + (x_right - x_left) * np.tan(beta_rad)
    ...
    if Hw > 0:
        ...
        y_wb_left = y_wl_left - Hw

        y_wl_right = y_top_right
        y_wb_right = y_wl_right - Hw
...
Th = st.sidebar.number_input("Heel thickness Th (m)", 0.2, 2.0, 0.8)
Tt = st.sidebar.number_input("Toe thickness Tt (m)", 0.2, 2.0, 0.6)

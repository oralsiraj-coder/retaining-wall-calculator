...
def draw_wall(Ha, Hw, Hp, Th, Lh, Lt, Tsb, beta,
    ...
    if show_labels:
        draw_dimension(ax, (x0, y0), (x0, y0+Th_s), "Th", offset=-0.5, vertical=True)
        draw_dimension(ax, (x0, y0), (x0+Lh_s, y0), "Lh", offset=-0.6)
        draw_dimension(ax, (x0+Lh_s+Tsb_s, y0), (x0+Lh_s+Tsb_s+Lt_s, y0), "Lt", offset=-0.6)
        draw_dimension(ax, (x0+Lh_s, y0+Th_s), (x0+Lh_s+Tsb_s, y0+Th_s), "Tsb", offset=0.3)
        draw_dimension(ax, (x0, y0+Th_s), (x0, y0+Th_s+Ha_s), "Ha", offset=-0.7, vertical=True)
        draw_dimension(ax,
            (x0+Lh_s+Tsb_s+Lt_s, y0+Th_s),
            (x0+Lh_s+Tsb_s+Lt_s, y0+Th_s+Hp_s),
            "Hp", offset=0.7, vertical=True)

        ax.text((xL+xR)/2, (yTL+yTR)/2+0.15, f"β={beta:.0f}°",
    ...
    legend_items = [
        ...
        Line2D([0],[0], linestyle="--", color="black", label="Ground surface")
    ]
    ax.legend(handles=legend_items, loc="upper right", fontsize=8, title="Legend")
...
def generate_input_report_pdf(
    ...
    for txt in [
        ...
        f"Th = {Th:.2f} m   (Base thickness)",
        f"Lh = {Lh:.2f} m   (Heel length)",
        f"Lt = {Lt:.2f} m   (Toe length)",
        f"Tsb = {Tsb:.2f} m (Stem thickness)",
        f"β = {beta:.1f}°   (Backfill slope)",
        "",
        f"Active soil: γ={gamma_a:.1f}, φ={phi_a:.1f}°, c={c_a:.1f}",
        f"Passive soil: γ={gamma_p:.1f}, φ={phi_p:.1f}°, c={c_p:.1f}",
    ]:
        line(txt)
...
# ============================================================
# STREAMLIT UI (SAFE ORDER)
# ============================================================
st.title("🧱 Retaining Wall – Geometry & Input Report")

...
mu = st.sidebar.number_input("Coefficient of friction between the soil and the base of the wall μ ", 0.0, 1.0, 0.0, 0.01)
...
Hp = st.sidebar.number_input("Height of passive soil (Hp)", 0.0, 20.0, 3.0)
...
ya1 = Ha / 3
st.latex(
    rf"P_{{a1}} = \frac{{1}}{{2}} K_a \gamma_a H_a^2"
    rf" = \frac{{1}}{{2}} \cdot {Ka:.3f} \cdot {gamma_a:.1f} \cdot {Ha:.1f}^2"
    rf" = {Pa1:.2f}"
    rf"\quad,\quad ")
st.latex(
    rf"y_{{a1}} = \frac{{H_a}}{{3}}"

...
def rankine_active(phi_deg, beta_deg):
    ...
    if beta > phi:
        st.error("Rankine invalid: β must be ≤ φ")
        st.stop()
...
def vertical_stress(z, soil, load, geom):
    ...
    sigma_soil = soil.gamma * (-z)
    sigma_surcharge = load.q * np.ones_like(z)
    sigma_water = -gamma_w * np.maximum(0, z_wt - z)

    sigma_effective = sigma_soil + sigma_water
    sigma_total = sigma_effective + sigma_surcharge

    return {
        "soil": sigma_soil,
        "surcharge": sigma_surcharge,
        "water": sigma_water,
        "effective": sigma_effective,
        "total": sigma_total,
    }
...
# ============================================================
def draw_wall(geom):
    scale = min(8 / (geom.Lh + geom.Tsb + geom.Lt), 8 / (geom.Ha + geom.Th))

    fig, ax = plt.subplots(figsize=(6, 6))

    ax.add_patch(Rectangle((2, 2), geom.Lh * scale + geom.Tsb * scale + geom.Lt * scale,
                           geom.Th * scale, fc="0.8"))

    ax.add_patch(Rectangle((2 + geom.Lh * scale, 2 + geom.Th * scale),
                           geom.Tsb * scale, geom.Ha * scale, fc="0.8"))
...
# STREAMLIT UI
# ============================================================
st.title("🧱 Retaining Wall Calculator")
...
FS = compute_forces(geom, soil_a, soil_p, load, Ka, Kp)

# Output
st.subheader(f"Ka = {Ka:.3f}, Kp = {Kp:.3f}")
st.subheader(f"Sliding FS = {FS:.2f}")

...
def rankine_active_coefficient(phi_deg, beta_deg):
    """
    Rankine active earth pressure coefficient (general form).
    Always used, including horizontal backfill (beta = 0).
    """
...
def rankine_passive_coefficient(phi_deg, beta_deg=0.0):
    ...
    Default beta = 0 (horizontal ground), but general expression is used.
    """
    phi = math.radians(phi_deg)
    beta = math.radians(beta_deg)
